...
class ParametersBasicNode(Node):
    ...
    def __init__(self, name):
        ...
        self.get_logger().info("开始创建 loop 线程")

        self.cam_thread = threading.Thread(target=self.loop)
        self.cam_thread.start()

        self.get_logger().info("启动了 loop 线程")
    ...
    def set_camera_parameters(self):
        ...
        print(f"设置的曝光时间: {self.cap.get(cv2.CAP_PROP_EXPOSURE)}")
        print(f"设置的自动曝光模式: {self.cap.get(cv2.CAP_PROP_AUTO_EXPOSURE)}")
    ...
    def timer_callback(self):
        ...
        if ret:
            ...
            print(f"图像大小: {frame.shape}, 帧率 FPS: {1 / (dt/1e9):2f}, 帧时间: {dt/1e6}")    
    ...
    def loop(self):
        ...
        while not self.cap.isOpened():
            time.sleep(0.5)
            cnt += 1
            print(f"摄像头打开失败，请检查摄像头是否正常连接！并重启节点 {cnt}")
            continue
        ...
        while rclpy.ok():
            ...
            if ret:
                ...
                print(f"图像大小: {frame.shape}, 帧率 FPS: {1 / (dt/1e9)}Hz , 帧时间: {dt/1e6} ms")    
